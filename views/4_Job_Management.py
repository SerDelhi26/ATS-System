...
def get_document_url(file_path):
    try:
        response = supabase.storage.from_("job_documents").create_signed_url(file_path, 3600)
        return response["signedURL"]
    except:
        return None

def upload_job_document(uploaded_file, file_name):
    try:
        file_bytes = uploaded_file.getvalue()
        supabase.storage.from_("job_documents").upload(file_name, file_bytes)
        return file_name
...
if is_admin:
    with left_col:
        ...
        selected_job_title = st.selectbox(
            "Job Title",
            job_title_names,
            index=0 if not editing_job else job_title_names.index(default_job_title) if default_job_title in job_title_names else 0,
            key=get_key("job_title")
        )

        if st.checkbox("Add New Job Title", key=get_key("add_new_title")):
            new_job_title = st.text_input("New Job Title", key=get_key("new_title_input"))
            if st.button("Save Job Title", key=get_key("save_title_btn")):
                existing = supabase.table("job_title_master").select("*").ilike("job_title_name", new_job_title).execute()
                if existing.data:
        ...
        selected_company = st.selectbox(
            "Company",
            company_names,
            index=company_names.index(default_company) if editing_job and default_company in company_names else 0,
            key=get_key("company")
        )

        if st.checkbox("Add New Company", key=get_key("add_new_company")):
            new_company = st.text_input("New Company", key=get_key("new_company_input"))
            if st.button("Save Company", key=get_key("save_company_btn")):
                existing = supabase.table("company_master").select("*").ilike("company_name", new_company).execute()
                if existing.data:
        ...
        selected_category_name = st.selectbox(
            "Category",
            category_names,
            index=category_names.index(default_category) if editing_job and default_category in category_names else 0,
            key=get_key("category")
        )

        if st.checkbox("Add New Category", key=get_key("add_new_cat")):
            new_category = st.text_input("New Category", key=get_key("new_cat_input"))
            if st.button("Save Category", key=get_key("save_cat_btn")):
                existing = supabase.table("category_master").select("*").ilike("category_name", new_category).execute()
                if existing.data:
        ...
        selected_sub_category = st.selectbox(
            "Sub Category",
            sub_category_names,
            index=sub_category_names.index(default_sub_category) if editing_job and default_sub_category in sub_category_names else 0,
            ...
            try:
                ...
                if editing_job:
                    ...
                    if job_document:
                        ...
                        job_document_path = upload_job_document(job_document, unique_file_name)
...
# ==========================
# RIGHT PANEL (JOB DIRECTORY)
# ==========================
with right_col:
    st.markdown("## 📋 Job Directory")

    if st.session_state.job_document_url:
        st.link_button("📄 Open Selected Job Document", st.session_state.job_document_url, use_container_width=True)
...
# Apply Search & Dropdown Filters
    ...
    if not jobs_df.empty:
        ...
        display_jobs_df = jobs_df.head(25).fillna("")
        
        if total_jobs > 25:
            st.caption(f"⚠️ Showing top 25 of {total_jobs} results. Use search/filters to find specific records.")

        # Setup dynamic column widths depending on role
        if is_admin:
        ...
        for _, row in display_jobs_df.iterrows():
            with st.container():
                ...
                cols[3].write(", ".join(job_recruiters))
                ...
                if row["job_document_path"]:
                    if cols[7].button("📄 View", key=f"doc_{row['job_id']}"):
                        st.session_state.job_document_url = get_document_url(row["job_document_path"])
                        st.rerun()
                else:
                    cols[7].write("-")
